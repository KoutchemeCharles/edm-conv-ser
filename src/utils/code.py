"""
Python code analysis utilities: AST parsing, compilation check, normalization.
"""
...
def get_bytecode_representation(code):
    """Compile ``code`` and return its raw bytecode object."""
...
def does_compile(code):
    """Return True if ``code`` parses successfully as Python."""
    return get_ast(code) is not None


def get_ast(code):
    """
    Parse ``code`` and return the AST, or ``None`` on syntax error.
...
def simple_clean(s):
    """
    Round-trip ``s`` through astor to produce a normalized source string.
